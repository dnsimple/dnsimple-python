...
from dnsimple.struct import Certificate
from dnsimple.struct import LetsencryptCertificateInput
from dnsimple.struct import LetsencryptCertificateRenewalInput
...
class CertificatesTest(DNSimpleTest):
    @responses.activate
    def test_list_certificates(self):
        responses.add(
            DNSimpleMockResponse(
                method=responses.GET,
                path="/1010/domains/example.com/certificates",
                fixture_name="listCertificates/success",
            )
        )
        certificates = self.certificates.list_certificates(1010, "example.com").data

        self.assertEqual(2, len(certificates))
        self.assertIsInstance(certificates[0], Certificate)

    @responses.activate
    def test_list_certificates_supports_pagination(self):
        responses.add(
            DNSimpleMockResponse(
                method=responses.GET,
                path="/1010/domains/example.com/certificates?page=1&per_page=2",
                fixture_name="listCertificates/success",
            )
        )
        response = self.certificates.list_certificates(
            1010, "example.com", page=1, per_page=2
        )
    ...
    def test_get_certificate(self):
        responses.add(
            DNSimpleMockResponse(
                method=responses.GET,
                path="/1010/domains/example.com/certificates/1",
        ...
        self.assertIsInstance(
            datetime.datetime.strptime(certificate.created_at, "%Y-%m-%dT%H:%M:%SZ"),
            datetime.datetime,
        )
        self.assertIsInstance(
            datetime.datetime.strptime(certificate.updated_at, "%Y-%m-%dT%H:%M:%SZ"),
            datetime.datetime,
        )
        self.assertIsInstance(
            datetime.datetime.strptime(certificate.expires_on, "%Y-%m-%d"),
            datetime.datetime,
        )
    ...
    def test_download_certificate(self):
        responses.add(
            DNSimpleMockResponse(
                method=responses.GET,
                path="/1010/domains/example.com/certificates/3/download",
                fixture_name="downloadCertificate/success",
            )
        )
        certificate_download = self.certificates.download_certificate(
            1010, "example.com", 3
        ).data

        self.assertEqual(
            "-----BEGIN CERTIFICATE-----\nMIIE7TCCA9WgAwIBAgITAPpTe4O3vjuQ9L4gLsogi"
            "/ukujANBgkqhkiG9w0BAQsF\nADAiMSAwHgYDVQQDDBdGYWtlIExFIEludGVybWVkaWF0ZSBYMTAeFw0xNjA2MTEx"
            ...
            "+YDvtnmDw23NcoTX7VSeCgU9M3RKs19AsCJcRNTLJ2dmD\nrAuyCTud9YTAaXQcTOLUhtO8T8"
            "+9AFVIva2OmAlKCR5saBW3JaRxW7V2aHEd/d1s\ns1CvNOO7jNppc9NwGSnDHcn3rqNv"
            "/U3MaU0gpJJRqsKkvcLU6IHJGgxyQ6AbpwJD"
            ...
            "+rkiZBTAfBgNVHSMEGDAWgBTAzANGuVggzFxycPPhLssgpvVoOjB4Bggr"
            ...
            "+BgNVHSAEgfYwgfMwCAYGZ4EMAQIBMIHmBgsrBgEEAYLf"
            ...
            "\nAEqMdWrmdIyQxthWsX3iHmM2h/wXwEesD0VIaA+Pq4mjwmKBkoPSmHGQ/O4v8RaK\nB6gl8v+qmvCwwqC1SkBmm"
            "+9C2yt/P6WhAiA/DD+WppYgJWfcz2lEKrgufFlHPukB\nDzE0mJDuXm09QTApWlaTZWYfWKY50T5uOT/rs+OwGFFCO"
            ...
            "=\n-----END CERTIFICATE-----\n",
            certificate_download.server,
        )
        self.assertIsNone(certificate_download.root)
        self.assertListEqual(
            [
                "-----BEGIN CERTIFICATE-----\nMIIEqzCCApOgAwIBAgIRAIvhKg5ZRO08VGQx8JdhT"
                ...
                "+bgixPg2MQ9k9oefFeqa+7Kqdlz5bbrUYV2volxhDFtnI4Mh8BiWCN\nxDH1Hizq+GKCcHsinDZWurCqder"
                "/afJBnQs+SBSL6MVApHt+d35zjBD92fO2Je56"
                ...
                "+4ni786iXwwFYNz8l3PmPLCyQXWGohnJ8iBm+5nk7O2ynaPVW0U2W+pt2w"
    ...
    @responses.activate
    def test_purchase_letsencrypt_certificate(self):
        responses.add(
            DNSimpleMockResponse(
        ...
        certificate_purchase = self.certificates.purchase_letsencrypt_certificate(
            1010, "example.com", LetsencryptCertificateInput(42)
        ).data
        ...
        self.assertIsInstance(
            datetime.datetime.strptime(
                certificate_purchase.created_at, "%Y-%m-%dT%H:%M:%SZ"
            ),
            datetime.datetime,
        )
        self.assertIsInstance(
            datetime.datetime.strptime(
                certificate_purchase.updated_at, "%Y-%m-%dT%H:%M:%SZ"
            ),
            datetime.datetime,
        )
    ...
    def test_issue_letsencrypt_certificate(self):
        ...
        certificate_issued = self.certificates.issue_letsencrypt_certificate(
            1010, "example.com", 101967
        ).data
    ...
    @responses.activate
    def test_purchase_letsencrypt_certificate_renewal(self):
        responses.add(
            DNSimpleMockResponse(
                method=responses.POST,
        ...
        renewal = self.certificates.purchase_letsencrypt_certificate_renewal(
            1010, "example.com", 200, LetsencryptCertificateRenewalInput(False, "RSA")
        ).data
        ...
        self.assertIsInstance(
            datetime.datetime.strptime(renewal.created_at, "%Y-%m-%dT%H:%M:%SZ"),
            datetime.datetime,
        )
        self.assertIsInstance(
            datetime.datetime.strptime(renewal.updated_at, "%Y-%m-%dT%H:%M:%SZ"),
            datetime.datetime,
        )
    ...
    def test_issue_letsencrypt_certificate_renewal(self):
        responses.add(
            DNSimpleMockResponse(
                method=responses.POST,
                path="/1010/domains/example.com/certificates/letsencrypt/300/renewals/999"
                "/issue",
                fixture_name="issueRenewalLetsencryptCertificate/success",
            )
        )
        certificate = self.certificates.issue_letsencrypt_certificate_renewal(
            1010, "example.com", 300, 999
        ).data
