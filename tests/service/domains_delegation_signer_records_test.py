...
from dnsimple.struct import DelegationSignerRecord
from dnsimple.struct import DelegationSignerRecordInput
...
class DomainsDelegationSignerRecordsTest(DNSimpleTest):
    ...
    def test_list_domain_delegation_signer_records(self):
        ...
        delegation_signer_records = self.domains.list_domain_delegation_signer_records(
            1010, 1
        ).data
    ...
    def test_list_domain_delegation_signer_records_supports_sorting(self):
        responses.add(
            DNSimpleMockResponse(
                method=responses.GET,
                path="/1010/domains/1/ds_records?sort" "=created_at:asc",
                fixture_name="listDelegationSignerRecords/success",
            )
        )
        self.domains.list_domain_delegation_signer_records(
            1010, 1, sort="created_at:asc"
        )
    ...
    def test_create_domain_delegation_signer_record(self):
        ...
        delegation_signer_record_input = DelegationSignerRecordInput(
            algorithm="13",
        ...
        delegation_signer_record = self.domains.create_domain_delegation_signer_record(
            1010, 1, delegation_signer_record_input
        ).data

        self.assertEqual(1010, delegation_signer_record.domain_id)
        self.assertEqual(
            delegation_signer_record_input.get("algorithm"),
            delegation_signer_record.algorithm,
        )
    ...
    def test_create_domain_delegation_signer_record_fails_validation(self):
        ...
        try:
            delegation_signer_record_input = DelegationSignerRecordInput(
                algorithm=None, digest=None, digest_type=None, keytag=None
            )

            self.domains.create_domain_delegation_signer_record(
                1010, 1, delegation_signer_record_input
            )
        except DNSimpleException as dnse:
            self.assertEqual(dnse.message, "Validation failed")
            self.assertEqual(
                "can't be blank", dnse.attribute_errors.get("algorithm")[0]
            )
