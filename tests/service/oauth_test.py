...
class OauthTest(DNSimpleTest):
    ...
    def test_exchange_authorization_for_token(self):
        ...
        oauth_token = self.oauth.exchange_authorization_for_token(self.code, self.client_id, self.client_secret,
                                                                  self.state, self.redirect_uri).data
    ...
    def test_authorize_url(self):
        ...
        self.assertEqual('https://api.sandbox.dnsimple.com/oauth/authorize?client_id=great-app&response_type=code', url)

    def test_authorize_url_with_params(self):
        url = self.oauth.authorize_url('great-app', redirect_uri='https://example.com', state='secret')

        self.assertEqual('https://api.sandbox.dnsimple.com/oauth/authorize?client_id=great-app&response_type=code'
                         '&redirect_url=https%3A%2F%2Fexample.com&state=secret', url)
